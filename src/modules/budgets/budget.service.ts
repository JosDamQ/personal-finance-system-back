...
export class BudgetService {
  ...
  async createBudget(userId: string, data: CreateBudgetDto): Promise<BudgetResponse> {
    ...
    const exists = await this.budgetRepository.checkMonthYearExists(
      userId,
      data.month,
      data.year,
    );
  ...
  async getBudgetByMonthYear(
    ...
    const budgetWithExpenses = await this.budgetRepository.getBudgetWithExpenses(
      budget.id,
      userId,
    );
  ...
  async updateBudget(
    ...
    if (
      ...
      !["BIWEEKLY", "MONTHLY"].includes(data.paymentFrequency)
    ) {
      throw new Error("Invalid payment frequency");
    }
    ...
    return budgetWithExpenses
      ? this.mapToResponseWithExpenses(budgetWithExpenses)
      : this.mapToResponse(updatedBudget);
  }
  ...
  async copyFromPreviousMonth(
    ...
    const previousBudget = await this.budgetRepository.findByMonthYear(
      userId,
      prevMonth,
      prevYear,
    );
  ...
  private validatePeriods(
    frequency: string,
    periods: { periodNumber: number; income: number }[],
  ): void {
