...
export class AlertService {
  ...
  async createAlert(userId: string, data: CreateAlertDto): Promise<AlertResponse> {
    ...
    const existingAlert = await this.alertRepository.findExistingAlert(userId, data.type, data.metadata);
    if (existingAlert) {
    ...
    await this.notificationService.sendNotification(userId, data.type, data.title, data.message, data.metadata);
  ...
  async checkAndCreateCreditLimitWarnings(userId: string): Promise<AlertResponse[]> {
    ...
    for (const warning of warnings) {
      const metadata: CreditLimitWarningMetadata = {
        ...
        currentBalance: warning.currency === 'GTQ' ? warning.creditCard.currentBalanceGTQ : warning.creditCard.currentBalanceUSD,
        limit: warning.currency === 'GTQ' ? warning.creditCard.limitGTQ : warning.creditCard.limitUSD,
  ...
  async checkAndCreateBudgetExceededAlert(userId: string, budgetPeriodId: string): Promise<AlertResponse | null> {
    try {
      ...
      const totalExpenses = budgetPeriod.expenses.reduce((sum: number, expense: any) => sum + expense.amount, 0);
  ...
  async createPaymentReminder(userId: string, creditCardId: string, dueDate: Date, minimumPayment?: number): Promise<AlertResponse> {
    const creditCard = await this.creditCardService.getCreditCardById(creditCardId, userId);
  ...
  async createMonthlySummary(userId: string, month: number, year: number): Promise<AlertResponse> {
  ...
  async cleanupOldAlerts(userId: string, daysOld: number = 30): Promise<{ deletedCount: number }> {
  ...
  private async getBudgetPeriodWithExpenses(budgetPeriodId: string, userId: string): Promise<any | null> {
    try {
