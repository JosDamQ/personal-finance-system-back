...
import { CreateCreditCardDto, UpdateCreditCardDto, CreditCardResponse } from "./credit-card.types";
...
export class CreditCardService {
  ...
  async createCreditCard(userId: string, data: CreateCreditCardDto): Promise<CreditCardResponse> {
  ...
  async updateCreditCard(id: string, userId: string, data: UpdateCreditCardDto): Promise<CreditCardResponse> {
    ...
    if (data.name && data.name !== existingCreditCard.name) {
      const nameExists = await this.creditCardRepository.checkNameExists(userId, data.name, id);
      if (nameExists) {
        throw new Error('Credit card name already exists');
      }
    }
  ...
  async updateCreditCardBalance(id: string, userId: string, amountGTQ?: number, amountUSD?: number): Promise<CreditCardResponse> {
    // Verificar que la tarjeta existe
    const existingCreditCard = await this.creditCardRepository.findById(id, userId);
    if (!existingCreditCard) {
      throw new Error('Credit card not found');
    ...
    const updatedCreditCard = await this.creditCardRepository.updateBalance(id, userId, updateData);
    return this.mapToResponse(updatedCreditCard);
  }

  async checkCreditLimitWarning(id: string, userId: string): Promise<{ needsWarning: boolean; currency?: 'GTQ' | 'USD'; percentage?: number }> {
    const creditCard = await this.creditCardRepository.findById(id, userId);
    if (!creditCard) {
      throw new Error('Credit card not found');
    }
  ...
  async getCardsNeedingWarnings(userId: string): Promise<Array<{ creditCard: CreditCardResponse; currency: 'GTQ' | 'USD'; percentage: number }>> {
    const creditCards = await this.creditCardRepository.findByUserId(userId);
    const warnings: Array<{ creditCard: CreditCardResponse; currency: 'GTQ' | 'USD'; percentage: number }> = [];
  ...
  async getCreditSummary(userId: string): Promise<{ totalLimitGTQ: number; totalLimitUSD: number; totalUsedGTQ: number; totalUsedUSD: number; totalAvailableGTQ: number; totalAvailableUSD: number; activeCards: number }> {
    const creditCards = await this.creditCardRepository.findByUserId(userId);
    const activeCards = creditCards.filter(card => card.isActive);

    const summary = activeCards.reduce((acc, card) => {
