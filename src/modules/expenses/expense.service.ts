...
import { CreateExpenseDto, UpdateExpenseDto, ExpenseResponse, ExpenseFilters, ExpenseSummary } from "./expense.types";
...
export class ExpenseService {
  ...
  async getUserExpenses(userId: string, filters?: ExpenseFilters): Promise<ExpenseResponse[]> {
  ...
  async updateExpense(id: string, userId: string, data: UpdateExpenseDto): Promise<ExpenseResponse> {
    ...
    if (data.categoryId) {
      const categoryExists = await this.expenseRepository.validateCategoryExists(data.categoryId, userId);
      if (!categoryExists) {
    ...
    if (data.creditCardId) {
      const creditCardExists = await this.expenseRepository.validateCreditCardExists(data.creditCardId, userId);
      if (!creditCardExists) {
    ...
    if (data.budgetPeriodId) {
      const budgetPeriodExists = await this.expenseRepository.validateBudgetPeriodExists(data.budgetPeriodId, userId);
    ...
    if (oldCreditCardId !== newCreditCardId) {
      ...
      if (oldCreditCardId) {
        await this.updateCreditCardBalance(oldCreditCardId, userId, -oldAmount, oldCurrency);
      }
      // Aplicar el gasto a la nueva tarjeta
      if (newCreditCardId) {
        await this.updateCreditCardBalance(newCreditCardId, userId, newAmount, newCurrency);
      }
    } else if (oldCreditCardId && (oldAmount !== newAmount || oldCurrency !== newCurrency)) {
  ...
  async getExpensesByCreditCard(creditCardId: string, userId: string): Promise<ExpenseResponse[]> {
    // Verificar que la tarjeta existe y pertenece al usuario
    const creditCardExists = await this.expenseRepository.validateCreditCardExists(creditCardId, userId);
    if (!creditCardExists) {
      throw new Error('Credit card not found');
    }
  ...
  async getExpensesByCategory(categoryId: string, userId: string): Promise<ExpenseResponse[]> {
    // Verificar que la categoría existe y pertenece al usuario
    const categoryExists = await this.expenseRepository.validateCategoryExists(categoryId, userId);
    if (!categoryExists) {
      throw new Error('Category not found');
    }
  ...
  async getExpensesByBudgetPeriod(budgetPeriodId: string, userId: string): Promise<ExpenseResponse[]> {
    // Verificar que el período de presupuesto existe y pertenece al usuario
    const budgetPeriodExists = await this.expenseRepository.validateBudgetPeriodExists(budgetPeriodId, userId);
    if (!budgetPeriodExists) {
      throw new Error('Budget period not found');
    }

    const expenses = await this.expenseRepository.findByBudgetPeriod(budgetPeriodId, userId);
    return expenses.map(this.mapToResponse);
  }

  async getExpensesSummary(userId: string, filters?: ExpenseFilters): Promise<ExpenseSummary> {
  ...
  private async validateExpenseData(userId: string, data: CreateExpenseDto): Promise<void> {
    // Validar categoría
    const categoryExists = await this.expenseRepository.validateCategoryExists(data.categoryId, userId);
    if (!categoryExists) {
      throw new Error('Category not found');
  ...
  private async updateCreditCardBalance(creditCardId: string, userId: string, amount: number, currency: string): Promise<void> {
    ...
    try {
      ...
      if (currency === 'GTQ') {
        ...
        await creditCardService.updateCreditCardBalance(creditCardId, userId, newBalance, undefined);
      } else if (currency === 'USD') {
        const newBalance = creditCard.currentBalanceUSD + amount;
        await creditCardService.updateCreditCardBalance(creditCardId, userId, undefined, newBalance);
      }
    } catch (error) {
